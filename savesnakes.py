...
# Setting up bg image
welcome_bg_image = pygame.image.load('Save the snakey (1).png')
welcome_bg_image = pygame.transform.scale(welcome_bg_image, (screen_width, screen_height)).convert_alpha()
gameover_bg_image = pygame.image.load('Game Over.png')
...
blue = (0, 0, 255)
yellow = (255, 255, 0)
pygame.display.set_caption("Save the snakey")  # Setting Game Heading
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    pygame.mixer.music.load('bg_song.mpeg')
                    pygame.mixer.Channel(0).play(pygame.mixer.Sound('bg_song.mpeg'))
                    gameloop()
        pygame.display.update()
        clock.tick(fps)
    ...
    while not exit_game:  # it doesnt lets the game to close
        if game_over:
            ...
            if abs(food_x - snake_x) < 12 and abs(food_y - snake_y) < 12:
                pygame.mixer.music.load('beep.mpeg')
                pygame.mixer.Channel(1).play(pygame.mixer.Sound('beep.mpeg'))
                score += 10
            ...
            if head in snake_list[:-1]:
                pygame.mixer.music.load('Game_over.mpeg')
